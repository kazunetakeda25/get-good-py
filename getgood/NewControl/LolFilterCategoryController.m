...
#import "LolFilterCategoryController.h"
#import "CategoryCell.h"

@interface LolFilterCategoryController ()

@property (nonatomic, strong) NSString* prefix;
@end

@implementation LolFilterCategoryController
...
- (void)viewDidLoad {
    ...
    {
        strCategory = [defult objectForKey:[NSString stringWithFormat:@"%@category_player", self.prefix]];
        self.bRoleSelect = [defult boolForKey:[NSString stringWithFormat:@"%@category_player_role", self.prefix]];
    }
    else if([Temp currentTab] == 1)
    {
        strCategory = [defult objectForKey:[NSString stringWithFormat:@"%@category_group", self.prefix]];
        self.bRoleSelect = [defult boolForKey:[NSString stringWithFormat:@"%@category_group_role", self.prefix]];
    }
    else if([Temp currentTab] == 2)
    {
        strCategory = [defult objectForKey:[NSString stringWithFormat:@"%@category_coach", self.prefix]];
        self.bRoleSelect = [defult boolForKey:[NSString stringWithFormat:@"%@category_coach_role", self.prefix]];
    }
    ...
    for(int i = 0; i < ArrCategory2.count; i++)
    {
        ...
        if([strCategory containsString:[category objectForKey:@"Image"]])
        {
...
- (IBAction)btnDoneClick:(id)sender {
    ...
    if([Temp currentTab] == 0)
    {        
        [deflult setValue:strCategory forKey:[NSString stringWithFormat:@"%@category_player", self.prefix]];
        [deflult setBool:self.bRoleSelect forKey:[NSString stringWithFormat:@"%@category_player_role", self.prefix]];
    }
    else if([Temp currentTab] == 1)
    {
        [deflult setValue:strCategory forKey:[NSString stringWithFormat:@"%@category_group", self.prefix]];
        [deflult setBool:self.bRoleSelect forKey:[NSString stringWithFormat:@"%@category_group_role", self.prefix]];
    }
    else if([Temp currentTab] == 2)
    {
        [deflult setValue:strCategory forKey:[NSString stringWithFormat:@"%@category_coach", self.prefix]];
        [deflult setBool:self.bRoleSelect forKey:[NSString stringWithFormat:@"%@category_coach_role", self.prefix]];
    }
